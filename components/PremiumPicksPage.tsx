...
/**
 * Parse the enriched prediction string produced by morning-picks:
 *   "Bayern vs Dortmund — 1 @ 1.44 | Bundesliga | conf:78 [✓Vista+Vital]"
...
function parsePrediction(raw: string): {
    ...
    const teams = dashIdx >= 0 ? main.slice(0, dashIdx).trim() : main;
    const afterDash = dashIdx >= 0 ? main.slice(dashIdx + 3).trim() : "";
    // afterDash = "1 @ 1.44"
    const [tipPart, oddPart] = afterDash.split(" @ ");
    const tip = tipPart?.trim() ?? "";
    const odd = oddPart?.trim() ?? "";

    // Extract confidence number
    const confMatch = confPart.match(/conf:(\d+)/);
    const conf = confMatch ? parseInt(confMatch[1], 10) : null;

    // Extract cross-validation sources
    const cvMatch = confPart.match(/\[✓([^\]]+)\]/);
    const crossVal = cvMatch
        ? cvMatch[1].split("+").map((s) => s.trim()).filter(Boolean)
        : [];

    return { teams, tip, odd, league: leaguePart.trim(), conf, crossVal };
}
...
// ─── Tier Badge ────────────────────────────────────────────────────────────────
const TierBadge = ({ tier }: { tier: PickTier }) => {
    if (!tier) return null;
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, flexWrap: "wrap" }}>
                            ...
                            {meta && (
                                <span style={{ fontSize: 9, color: meta.color, background: meta.bg, border: `1px solid ${meta.border}`, padding: "2px 7px", borderRadius: 3, fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase" }}>
                                    {meta.emoji} {meta.label}
                                </span>
                            )}
                        </div>
        ...
        <div style={{ position: "relative" }}>
            {pick.matches.map((_, i) => (
                <div key={i} style={{
                    ...
                    filter: "blur(5px)", userSelect: "none", pointerEvents: "none", opacity: 0.45,
                }}>
                    <div style={{ fontSize: 13, color: "#E8EAF0", lineHeight: 1.4, background: "#2A3140", borderRadius: 6, height: 18, width: `${60 + (i % 3) * 15}%` }} />
            ...
            <div style={{
                ...
                {view === "detail" && (
                    <>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: pick.avg_confidence != null ? 12 : 20, paddingBottom: pick.avg_confidence != null ? 12 : 20, borderBottom: pick.avg_confidence != null ? "none" : "1px solid #2A3140" }}>
                            <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 36, color: tierMeta?.color ?? "#C9A84C" }}>x{pick.total_odds}</span>
                            <div>
