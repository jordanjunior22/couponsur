    ...
    <div style={{ minHeight: "100vh", background: C.dark }} className="buyer-page">
      ...
      <div style={{ padding: 24 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          {/* Section Picks en direct - uniquement les picks PENDING */}
          {(!user || !user.hasActiveSubscription) && (
            <>
              {/* Picks EN COURS (PENDING) - avec boutons d'accès */}
              {picks.filter((p) => p.outcome === "PENDING").length > 0 && (
                    ...
                    {picks.filter((p) => p.outcome === "PENDING").map((pick) => (
                      <div key={pick._id} style={{ background: C.dark3, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, overflow: "hidden" }}>
                        ...
                        <div style={{ marginBottom: 12 }}>
                          ...
                          <h3 style={{ fontSize: 14, fontWeight: 600, color: C.text, margin: 0, marginBottom: 8 }}>{pick.title}</h3>
                          <div style={{ fontSize: 12, color: C.muted, marginBottom: 4 }}>{pick.league} · {formatDate(pick.match_date)}</div>
                          {/* Coupon Code */}
                        ...
                        {/* Prix visible pour inciter à payer */}
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
                          <div style={{ background: C.dark4, borderRadius: 6, padding: 8, textAlign: "center" }}>
                            ...
                            <div style={{ fontSize: 12, fontWeight: 600, color: C.gold }}>{formatCFA(pick.price)}</div>
                          </div>
                          <div style={{ background: C.dark4, borderRadius: 6, padding: 8, textAlign: "center" }}>
                            <div style={{ fontSize: 10, color: C.muted, marginBottom: 4, textTransform: "uppercase" }}>Cotes</div>
                            <div style={{ fontSize: 12, fontWeight: 600, color: C.gold }}>{pick.total_odds.toFixed(2)}</div>
                          </div>
                        </div>

                        {!user && (
                          <a href="/billing" style={{ display: "block", fontSize: 11, color: C.gold, textAlign: "center", padding: 10, background: "rgba(201,168,76,0.1)", borderRadius: 6, border: `1px solid ${C.gold}40`, textDecoration: "none", fontWeight: 600, cursor: "pointer" }}>
              ...
              {/* Picks avec résultats (WIN/LOSS) - détails visibles, pas de bouton */}
              {picks.filter((p) => p.outcome !== "PENDING").length > 0 && (
                <div style={{ marginTop: 24, paddingTop: 24, borderTop: `2px solid ${C.border}` }}>
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: "9px", letterSpacing: "2px", color: C.gold, textTransform: "uppercase", fontWeight: 600, marginBottom: 8 }}>
                      Résultats
                    </div>
                    <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "clamp(20px, 6vw, 28px)", color: C.text, letterSpacing: 1, margin: 0 }}>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    {pick.couponCode && (user?.hasActiveSubscription || unlockedPicks.includes(pick._id)) && (
                      <div style={{
                        ...
                        Code {pick.couponCode.broker}: <strong>{pick.couponCode.code}</strong>
                      </div>
                    )}
                  </div>
      ...
      <style>{`
        ...
        @media (max-width: 639px) {
          ...
          .buyer-page [style*="marginBottom: 32"] {
            margin-bottom: 20px !important;
          }

          .buyer-page [style*="marginTop: 40"] {
            margin-top: 24px !important;
          }

          .buyer-page [style*="paddingTop: 40"] {
