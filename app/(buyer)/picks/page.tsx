...
export default function PicksPage() {
  ...
  const displayedPicks = (() => {
    ...
    if (!user || !user.hasActiveSubscription) {
      // Show all PENDING picks (to unlock) + last 5 WIN picks (as proof)
                    ...
                    {group.picks.map((pick) => (
                      ...
                      >
                        ...
                        {pick.pickType === "SIMPLE" && pick.matches && pick.matches.length > 0 && (() => {
                          const isLocked = pick.outcome === "PENDING" && !user?.hasActiveSubscription && !unlockedPicks.includes(pick._id);
                          return (
                            <div style={{ position: "relative", marginBottom: 12 }}>
                              <div style={{
                                ...
                                {pick.matches.slice(0, 2).map((m: Match, i: number) => (
                                  <div key={i} style={{ fontSize: 11, color: C.text, marginBottom: 6, display: "flex", flexDirection: "column", gap: 2 }}>
                                    {m.teams && (
                              ...
                              {isLocked && (
                                <div style={{
                                  ...
                                  <span style={{ fontSize: 11, color: C.muted }}>🔒 Débloque pour voir</span>
                                </div>
                              )}
                            </div>
                          );
