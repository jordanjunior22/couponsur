...
function SettingsTab() {
  ...
  return (
    <div style={{ maxWidth: 480, display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <GroupChatAdminCard
        room="premium"
        settingsField="groupChatEnabled"
        title="Groupe Premium"
        description="Discussion de groupe réservée aux admins et aux abonnés actifs, accessible depuis l&apos;onglet Chat (👑) de l&apos;app."
      />

      <GroupChatAdminCard
        room="global"
        settingsField="globalChatEnabled"
        title="Chat Global"
...
// ─── Group chat room admin card ─────────────────────────────────────────────
// Same enable-toggle + storage-stats + "wipe all" lever for either room —
// fully self-contained (its own settings/stats fetch) so SettingsTab above
// doesn't have to thread six pieces of per-room state through its own
// shared fetch effect for what's otherwise an independent panel.
function GroupChatAdminCard({
  room, settingsField, title, description,
}: {
  room: "premium" | "global";
  settingsField: "groupChatEnabled" | "globalChatEnabled";
  title: string;
  description: string;
}) {
  const [enabled, setEnabled] = useState(true);
  const [saving, setSaving] = useState(false);
  const [saveMsg, setSaveMsg] = useState<string | null>(null);
  const [stats, setStats] = useState<{ messageCount: number; imageCount: number; totalBytes: number } | null>(null);
  const [statsLoading, setStatsLoading] = useState(true);
  const [clearing, setClearing] = useState(false);

  const fetchStats = async () => {
    try {
      const res = await fetch(`/api/admin/group-chat/stats?room=${room}`, { credentials: "include" });
      const data = await res.json();
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (data?.success) setEnabled(data.data[settingsField] !== false);
      } catch (e) {
        console.error("Settings fetch:", e);
      }
    })();
  ...
  const saveEnabled = async (next: boolean) => {
    ...
    try {
      const res = await fetch("/api/admin/settings", {
        ...
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ [settingsField]: next }),
      });
      const data = await res.json();
      if (data?.success) {
        setEnabled(data.data[settingsField] !== false);
        setSaveMsg("Enregistré avec succès.");
      } else {
        setEnabled(!next); // revert the optimistic toggle
        setSaveMsg(data.message || "Erreur lors de l'enregistrement.");
      }
    } catch {
      setEnabled(!next);
  ...
  const clear = async () => {
    if (!confirm(`Supprimer définitivement tous les messages de "${title}" (y compris les images) ? Cette action est irréversible.`)) return;
    setClearing(true);
    try {
      const res = await fetch(`/api/admin/group-chat/clear?room=${room}`, { method: "DELETE", credentials: "include" });
      const data = await res.json();
      if (data?.success) {
        setSaveMsg(`${data.data.deletedCount} message(s) supprimé(s).`);
        await fetchStats();
      } else {
        setSaveMsg(data.message || "Échec de la suppression.");
      }
    } catch {
      setSaveMsg("Erreur réseau lors de la suppression.");
    } finally {
      setClearing(false);
  ...
  return (
    <div style={{ background: C.dark3, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
      ...
      {statsLoading ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 16 }}>
          <div style={{ background: C.dark4, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px" }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: C.text }}>{stats.messageCount}</div>
            <div style={{ fontSize: 10, color: C.muted }}>messages</div>
          </div>
          <div style={{ background: C.dark4, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px" }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: C.text }}>{stats.imageCount}</div>
            <div style={{ fontSize: 10, color: C.muted }}>images</div>
          </div>
          <div style={{ background: C.dark4, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px" }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: C.gold }}>{formatBytes(stats.totalBytes)}</div>
      ...
      <button
        onClick={clear}
        disabled={clearing || !stats?.messageCount}
        style={{
          ...
          cursor: (clearing || !stats?.messageCount) ? "not-allowed" : "pointer",
          opacity: (clearing || !stats?.messageCount) ? 0.5 : 1,
        }}
      >
        {clearing ? "Suppression…" : "🗑 Vider toutes les discussions"}
      </button>

      {saveMsg && (
        <div style={{
          fontSize: 12, padding: "8px 12px", borderRadius: 6, marginTop: 12,
          background: saveMsg.includes("succès") || saveMsg.includes("supprimé") ? "rgba(34,197,94,0.08)" : "rgba(239,68,68,0.08)",
          border: `1px solid ${saveMsg.includes("succès") || saveMsg.includes("supprimé") ? "rgba(34,197,94,0.25)" : "rgba(239,68,68,0.25)"}`,
          color: saveMsg.includes("succès") || saveMsg.includes("supprimé") ? C.green : C.red,
        }}>
          {saveMsg}
        </div>
      )}
